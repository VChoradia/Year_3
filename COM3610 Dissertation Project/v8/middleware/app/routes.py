...
def init_routes(app):
                ...
                while True:
                    try:
                        ...
                        if latest_data:
                            ...
                            yield f"data:{json.dumps({'bpm': 'No data', 'temperature': 'No data'})}\n\n"
                        time.sleep(5) 
                    except Exception as e:
                        app.logger.error(f"Error retrieving device data: {str(e)}")
                        yield f"data:{json.dumps({'bpm': 'Error', 'temperature': 'Error'})}\n\n"
                        time.sleep(5)
    ...
    def get_users():
        ...
        users_list = [{
            'id': user.id,
            'username': user.username,
            'device_assigned': user.device.mac_address if user.device else None  
        } for user in users]
