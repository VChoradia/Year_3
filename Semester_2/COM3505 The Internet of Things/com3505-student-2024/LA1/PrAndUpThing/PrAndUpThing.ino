// Lab Assignment 1
// Author(s): Vivek Choradia, Siya Pradhan
// Multiple code snippets used from COM 3505 Internet of Things lab solutions authored by Prof. Hamish Cunningham
...
int firmwareVersion = 7; // used to check for updates

// Server's IP address and port number
#define FIRMWARE_SERVER_IP_ADDR "172.20.10.2"
...
void yellowLedOn()  { digitalWrite(YELLOW_LED, HIGH); }
void yellowLedOff() { digitalWrite(YELLOW_LED, LOW); }

void greenLedOn()  { digitalWrite(GREEN_LED, HIGH); }
void greenLedOff() { digitalWrite(GREEN_LED, LOW); }

void blinkRed(int times, int pause=100) {
  redLedOff();
  for(int i=0; i<times; i++) {
    redLedOn(); delay(pause); redLedOff(); delay(pause);
  }
}

void blinkYellow(int times, int pause=100) {
  yellowLedOff();
  for(int i=0; i<times; i++) {
    yellowLedOn(); delay(pause); yellowLedOff(); delay(pause);
  }
}

void blinkGreen(int times, int pause=100) {
  greenLedOff();
  for(int i=0; i<times; i++) {
    greenLedOn(); delay(pause); greenLedOff(); delay(pause);
    }
}
...
void setup() {
  ...
  while(!Serial); //until serial monitor starts

  getMAC(MAC_ADDRESS);  // store the MAC address as a chip identifier

  // Setting pin mode for all the LEDs
  pinMode(RED_LED, OUTPUT);       
  pinMode(GREEN_LED, OUTPUT);
  pinMode(YELLOW_LED, OUTPUT);
  pinMode(BUTTON_PIN, INPUT_PULLUP);

  dln(startupDBG, "\nsetup for Lab Assignment 1...");
  ...
  WiFi.begin();         
//   WiFi.begin(SSID, PSK);
  Serial.print("Wifi connection initiated");
  Serial.println("");
  blinkYellow(3); 
...
void loop() {
  ...
  if(digitalRead(BUTTON_PIN) == LOW) {
    ...
    for(int i=1; i <=3; i++) {
      ...
      Serial.println("Button held, initiating update...");
